...
import { range, SignalValue, createSignal, SimpleSignal, debug, createEffect, all, DEFAULT, createDeferredEffect, tween, chain, map, easeOutQuint, easeOutExpo, easeInOutExpo, waitFor } from '@motion-canvas/core';
...
export default makeScene2D(function* (view) {
    ...
    const Pair = (team1: string, team2: string, index: number) => (
            ...
            <Rect height={height} width={roundWidth[index]} lineWidth={5} stroke={'#88C0D0'} radius={rad}>
                <Layout direction={'column'} width={roundWidth[index]} height={height} gap={0} layout>
                    ...
                    <Layout direction={'column'} height={height - 50} gap={10} justifyContent={'center'} alignItems={'center'} layout>
                        {range(team).map((x) => Pair(teamPair[x].team1, teamPair[x].team2, index))}
                    </Layout>
                </Layout>
            </Rect>
        );
        ...
        (index: number) => {
            ...
            return (
                <Layout direction={'column'} width={roundWidth[index]} height={1000} gap={gapVertical} justifyContent={'center'} layout>
                    {groups.map((i) => Group(i.size, i.score, (heightWithoutGap * i.size) / teamCount, index)(data.find((x) => x.score === i.score).pairs))}
                </Layout>
            );
        };
